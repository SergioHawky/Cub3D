...
static void	set_draw_range(t_ray *ray, int *line_height, int *draw_start,
		int *draw_end)
{
...
static void	draw_column_pixels(t_data *data, int x, int draw_start,
		int draw_end, t_img *texture, int tex_x, float step, int line_height)
{
	int		y;
	int		tex_y;
	float	tex_pos;
	...
	while (y <= draw_end)
	{
		...
		ft_pixel_put(x, y++, &data->img, get_texture_pixel(texture, tex_x,
					tex_y));
		tex_pos += step;
	}
	while (y < HEIGHT)
...
static void	draw_column(t_data *data, t_ray *ray, int x)
{
	...
	draw_column_pixels(data, x, draw_start, draw_end, texture, tex_x, step,
			line_height);
}
